...
/* GET home page. */
// 요청이 들어오면 해석을 하는데
// 만약, get 방식이고, 주소가 / 이면 아래 함수가 호출된다 => 라이팅
router.get('/', function (req, res, next) {
  ...
  // index.ejs 파일을 읽어서 전달한 데이터를 사용해서 완성된 html을 응답한다 => 랜더링 한다
...
router.get('/search', function (req, res, next) {
  ...
  db.selectNoSellData((err, rows) => {
    ...
    // 3. search.ejs 내부에서 데이터를 반복해서 추출하여 화면을 구성한다. => search.ejs
    // 4. 만약 카테고리가 금융이면 box의 배경색을 bg-yellow로 처리한다.
...
// 동적 파라미터 url에 데이터를 넣어서 전송하는 방식 -> url은 무한대로 확장이 가능하다
// get방식, post방식 등 method 방식과 혼용해서 사용 가능
// ~/url/:변수명
